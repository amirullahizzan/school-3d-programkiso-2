...
extern Texture* getIblDiffuseTexture();  //temp
extern Texture* getIblSpecularTexture(); //temp
    ...
    for(s32 mesh_index = 0; mesh_index < MV1GetFrameMeshNum(mv1_handle_, frame_index); ++mesh_index) {    // フレームに含まれるメッシュの数
        ...
        for(s32 tlist_index = 0; tlist_index < MV1GetMeshTListNum(mv1_handle_, mesh); ++tlist_index) {    // メッシュに含まれるトライアングルリストの数
            ...
            // (2) 1フレーム前の(1)を設定してGeometryShaderで頂点合成
            {
                ...
                //assigning before rendering the model, or it might crash
                DxLib::SetUseTextureToShader(14, *getIblDiffuseTexture());
                DxLib::SetUseTextureToShader(15, *getIblSpecularTexture());

                // 描画 (本来のシーン用)
                MV1DrawTriangleList(mv1_handle_, tlist);

                DxLib::SetUseTextureToShader(14, -1);
                DxLib::SetUseTextureToShader(15, -1);
                //resetting to 0 after draw
